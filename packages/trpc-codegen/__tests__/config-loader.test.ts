...
import { DEFAULT_CONFIG, type TRPCCodegenConfig } from '../src/types/config.js';
...
describe('ConfigLoader', () => {
  ...
  describe('loadConfig()', () => {
    it('should load test config file successfully', async () => {
      const config = await ConfigLoader.loadConfig('./__tests__/fixtures/test-config.js', basePath);
      
      expect(config).toBeDefined();
      expect(config.source.sectorsDir).toBe(path.resolve(__dirname, 'fixtures/sectors'));
    ...
    it('should use default config when no config file is found', async () => {
      const config = await ConfigLoader.loadConfig(undefined, basePath);
      
      expect(config).toBeDefined();
      expect(config.source.sectorsDir).toBe(DEFAULT_CONFIG.source.sectorsDir);
      expect(config.generation.packageName).toBe(DEFAULT_CONFIG.generation.packageName);
      expect(config.zod2ts.enabled).toBe(DEFAULT_CONFIG.zod2ts.enabled);
    });

    it('should search multiple config file paths', async () => {
      // Test that it searches in the expected order
      const config = await ConfigLoader.loadConfig(undefined, basePath);
      
      // Should use default config when no config file is found
      expect(config.source.sectorsDir).toBe(DEFAULT_CONFIG.source.sectorsDir);
    });
  ...
  describe('mergeWithDefaults()', () => {
    it('should merge source configuration correctly', () => {
      const userConfig: Partial<TRPCCodegenConfig> = {
        source: {
          sectorsDir: 'custom/sectors',
          routerPattern: '*/custom/*.ts',
          schemaPattern: '*/custom/*.ts'
        }
      };
      
      const merged = ConfigLoader['mergeWithDefaults'](userConfig);
      
      expect(merged.source.sectorsDir).toBe('custom/sectors');
      expect(merged.source.routerPattern).toBe('*/custom/*.ts');
      expect(merged.source.schemaPattern).toBe('*/custom/*.ts');
    });

    it('should merge generation configuration correctly', () => {
      const userConfig: Partial<TRPCCodegenConfig> = {
        generation: {
          packageName: '@custom/package',
          outputDir: './custom',
          routerName: 'CustomRouter'
        }
      };
      
      const merged = ConfigLoader['mergeWithDefaults'](userConfig);
      
      expect(merged.generation.packageName).toBe('@custom/package');
      expect(merged.generation.outputDir).toBe('./custom');
      expect(merged.generation.routerName).toBe('CustomRouter');
    });

    it('should merge parsing configuration correctly', () => {
      const customPattern = /custom/;
      const userConfig: Partial<TRPCCodegenConfig> = {
        parsing: {
          endpointPattern: customPattern,
          routerMethodPattern: /custom/
        }
      };
      
      const merged = ConfigLoader['mergeWithDefaults'](userConfig);
      
      expect(merged.parsing.endpointPattern).toBe(customPattern);
      expect(merged.parsing.routerMethodPattern).toStrictEqual(/custom/);
    });

    it('should merge zod2ts configuration correctly', () => {
      const userConfig: Partial<TRPCCodegenConfig> = {
        zod2ts: {
          enabled: false,
      ...
      expect(merged.zod2ts.enabled).toBe(false);
      expect(merged.zod2ts.outputDir).toBe('./custom-types');
    });

    it('should handle empty user config', () => {
      const merged = ConfigLoader['mergeWithDefaults']({});
      
      expect(merged.source.sectorsDir).toBe(DEFAULT_CONFIG.source.sectorsDir);
      expect(merged.generation.packageName).toBe(DEFAULT_CONFIG.generation.packageName);
      expect(merged.zod2ts.enabled).toBe(DEFAULT_CONFIG.zod2ts.enabled);
    });
  });
});
