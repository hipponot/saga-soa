...
import { Zod2tsGenerator } from '../src/generators/zod2ts-generator.js';
...
describe('Zod2tsGenerator', () => {
  ...
  beforeEach(async () => {
    ...
    testConfig = await ConfigLoader.loadConfig(path.resolve(__dirname, 'fixtures/test-config.js'), basePath);
    ...
    for (const sector of sectors) {
      const sourceSchemaFile = path.join(basePath, 'sectors', sector.name, 'trpc', 'schema', `${sector.name}-schemas.ts`);
